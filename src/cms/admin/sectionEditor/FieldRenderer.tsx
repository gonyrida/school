...
import type { ImageRef, Button as ButtonType } from '@/cms/schema/sections';
...
/**
 * Renders one field according to its config. The data object is the
 * section's full data; we read/write to it via the field's path.
 */
export function FieldRenderer({ field, data, onChange }: Props) {
  const value = getAtPath(data, field.path);
  const set = (newValue: unknown) => onChange(setAtPath(data, field.path, newValue));
  ...
  switch (field.type) {
    ...
    case 'image':
      return <ImageField field={field} value={value as ImageRef | undefined} onChange={set} />;

    case 'button':
      return <ButtonField field={field} value={value as ButtonType | undefined} onChange={set} />;
    ...
    case 'repeater':
      return <RepeaterFieldRenderer field={field} value={(value as Array<Record<string, unknown>>) ?? []} onChange={set} />;
  }
}

function Label({ field, className = '' }: { field: FieldConfig; className?: string }) {
  return (
    <div className={`mb-1.5 ${className}`}>
      ...
      {field.description && <p className="text-xs text-ink-500">{field.description}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {hasImage ? (
          <div className="group relative overflow-hidden rounded-xl border border-ink-300/30 bg-surface-soft">
            <img src={value.url} alt={value.alt} className="h-40 w-full object-cover" />
            <div className="absolute inset-0 hidden items-center justify-center gap-2 bg-black/50 group-hover:flex">
              <button type="button" onClick={() => setPickerOpen(true)} className="rounded-lg bg-white px-3 py-1.5 text-sm font-medium">
                Replace
              </button>
              <button type="button" onClick={() => onChange(undefined)} className="rounded-lg bg-red-600 px-3 py-1.5 text-sm font-medium text-white">
                Remove
              </button>
            </div>
          </div>
      ...
      <MediaPicker
        ...
        onSelect={(item) => onChange({ url: item.url, alt: item.alt || value?.alt || '' })}
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() => onChange(enabled ? undefined : { label: 'Click me', href: '#', variant: 'primary' })}
      ...
      {enabled && (
        <div className="space-y-2 rounded-xl border border-ink-300/20 bg-surface-soft p-3">
          ...
          <select
            ...
            onChange={(e) => onChange({ ...current, variant: e.target.value as ButtonType['variant'] })}
            className="input-field !py-2 text-sm"
          >
...
function RepeaterFieldRenderer({
  ...
  const updateItem = (idx: number, newItem: Record<string, unknown>) => {
    const next = [...items];
    // Preserve id even if the inner editor doesn't include it
...
function RepeaterItem({
  ...
  // Try to derive a friendly preview label from the item
  const preview = (item.title || item.question || item.label || item.value || item.author || `${field.itemLabel} ${index + 1}`) as string;

  return (
    <div ref={setNodeRef} style={style} className="overflow-hidden rounded-xl border border-ink-300/20 bg-white">
      <div className="flex items-center gap-2 bg-surface-soft px-3 py-2">
        <button {...attributes} {...listeners} type="button" className="cursor-grab text-ink-400 hover:text-ink-700 active:cursor-grabbing">
        ...
        {onRemove && (
          <button type="button" onClick={onRemove} className="rounded-lg p-1 text-ink-400 hover:bg-red-50 hover:text-red-600">
...
// Suppress unused warning for X import retained for future use
void X;
